...
function breadcrumb_trail( $args = array() ) {
	...
	if ( !empty( $trail ) && is_array( $trail ) ) {
		...
		$breadcrumb = '<ul class="breadcrumbs hide-for-xsmall-only show-for-medium-up">';
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date() ) {
			...
			if ( is_day() ) {
				$trail[] = '<li><a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'templatic' ) ) . '">' . get_the_time( __( 'Y', 'templatic' ) ) . '</a></li>';
				$trail[] = '<li><a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'templatic' ) ) . '">' . get_the_time( __( 'F', 'templatic' ) ) . '</a></li>';
	...
	elseif ( is_search() )
		$trail[] = __( 'Search results for ', 'templatic' )."&quot;".esc_attr( get_search_query() )."&quot;";
...
function breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<li><a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'templatic' ), $post_id ) . '">' . get_the_time( __( 'Y', 'templatic' ), $post_id ) . '</a></li>';
			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<li><a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'templatic' ), $post_id ) . '">' . get_the_time( __( 'F', 'templatic' ), $post_id ) . '</a></li>';
			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<li><a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'templatic' ), $post_id ) . '">' . get_the_time( __( 'd', 'templatic' ), $post_id ) . '</a></li>';
