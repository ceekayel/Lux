...
if(isset($_REQUEST['pid']) && $_REQUEST['pid']!="" ){
        $terms = get_the_terms( $_REQUEST['pid'] , 'videoscategory' );
        if(!empty($terms)){
            foreach ( $terms as $term ) {
              $place_cat_arr[] = $term->term_id;
            }
        }
}else{
...
$catinfo = templ_get_video_parent_categories($taxonomy);
if(count($catinfo) == 0)
{
...
$terms = templ_get_video_parent_categories($taxonomy);

if($terms) :		
	
	$output .= '<ul>';
        $output .= '<label><input type="checkbox" name="selectall" id="selectall" onclick="displaychk();">'. __('Select All','templatic') .'</label>';
	foreach($terms as $term){	
		...
		$term_name = $term->name;
		if(isset($place_cat_arr) && in_array($term_id,$place_cat_arr)){ $selected = 'checked'; }else{ $selected='';} /* category must be selected when gobackand edit /Edit/Renew */
		$output .= '<li><label for="category-'.$term_id.'"><input type="checkbox" name="category[]" id="category-'.$term_id.'" value='.$term_id.','.$scp.' '.$selected.'/>'.$term_name.'</label></li>';
		
		$child_terms = templ_get_video_child_categories($taxonomy,$term_id);		/* get child categories term_id = parent id*/					
		$i=1;
		$parent_id = $term_id;
		$tmp_term_id=$term_id;
		foreach($child_terms as $child_term){ 
			...
			if($child_term->category_parent!=0):					
				...
				if(isset($place_cat_arr) && in_array($child_term_id,$place_cat_arr)){ $cselected = 'checked'; }else{ $cselected='';} /* category must be selected when gobackand edit /Edit/Renew */
				$output .= '<li><label for="category-'.$child_term_id.'"><input type="checkbox" name="category[]" id="category-'.$child_term_id.'" value='.$child_term_id.','.$child_cp.' '.$cselected.'/>'.$pad.$term_name.'</li>';	
			endif;
