...
/*
 update theme version after templatic member login
 */
 
function tmpl_update_theme()
{
	global $theme_name;
...
if( !class_exists('WPUpdates_Video_Updater') ) {
    class WPUpdates_Video_Updater {
    
		var $api_url;		
		var $theme_slug;
		
		/* Clear the data of previous updates */
		function tmpl_child_clear_update_transient() {
			$theme_name = basename(get_template_directory_uri());
			delete_transient( $theme_name.'-update' );
		}
		
		/* Function to set/check the auto updates */
		function __construct( $api_url,  $theme_slug ) {
			...
			/* check for updates if availabele in transient */
			add_filter( 'pre_set_site_transient_update_themes', array(&$this, 'check_for_update_') );
			add_action( 'after_theme_row_'.$theme_name, array(&$this, 'tmpl_theme_row') );
			add_action('wp_ajax_'.$theme_name,'tmpl_update_theme');
			...
			if(!strstr($_SERVER['REQUEST_URI'],'plugin-install.php') && !strstr($_SERVER['REQUEST_URI'],'update.php'))
			{
				...
				add_action( 'load-update-core.php', array(&$this,'tmpl_child_clear_update_transient') );
				add_action( 'load-themes.php', array(&$this, 'tmpl_child_clear_update_transient') );
				if(!strstr($_SERVER['REQUEST_URI'],'/network/')){
					add_action( 'admin_notices', array(&$this, 'tmpl_child_update_nag') );
				}
				delete_transient( $theme_name.'-update' );
    	...
    	function display_changelog(){	
			...
			$raw_response = wp_remote_request('http://templatic.com/updates/api/change_log.php?Video', $options);	
			if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
			  $page_text = __("Oops!! Something went wrong.<br/>Please try again or <a href='http://www.templatic.com'>contact us</a>.",'templatic');
			}
			else{
		...
		/* Function to display change log */
		function tmpl_child_update_nag($transient) {
			...
			{	
				echo '<div id="update-nag">';
				 $new_version = version_compare($theme_version, $remote_version, '<') ? __('There is a new version of Video available.', 'templatic')  : '';		
				 ...
				 $new_version .= ' <a class="thickbox" title="Video" href="'.$changelog.'">'. sprintf(__('View version %s Details', 'templatic'), $remote_version) . '</a>. Or ' ;
			          $theme_name = basename(get_template_directory_uri());
					...
					echo '<tr class="plugin-update-tr">
							<td colspan="3" class="plugin-update">
								<div class="update-message">' . $new_version . '<a href="'.$ajax_url.'" title="Video Update">'.__( 'update now','templatic').'</a></div>
							</td>
						 </tr>';
				echo '</div>';
			}
		...
		/*  Check the updates data are availabel or not */
		...
		function tmpl_theme_row()
		{
			...
			{	
				...
				echo '</tr><tr class="plugin-update-tr"><td colspan="3" class="plugin-update"><div class="update-message">' . $new_version .'<a href='.$ajax_url.' class="thickbox" title="Video Update">'.__( 'update now','templatic').'</a></div></td>';
...
/* css for autoupdates form*/
add_action('admin_print_styles','tmpl_admin_style');
function tmpl_admin_style(){ ?>
