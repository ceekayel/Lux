...
<div class='wrap templatic_login'>
  <?php if($flag ==1){ ?>
  <div id="update-nag">
    <h3 style=" clear:both;"> <?php echo sprintf(__('The new version of %s is available.','templatic'),$theme_name); ?></h3>
    <p>
      <?php echo __('you can update to the latest version automatically , or download the latest version of the theme.','templatic'); ?>
    </p>
    <p><span style="color:red; font-weight:bold;">
      <?php echo __('Warning','templatic'); ?>
      : </span>
      <?php echo __('Remember that the updates will replace all your changes so please keep backup of your changes.','templatic'); ?>
    </p>
    <a class="button-primary" href="http://templatic.com/members/member" target="blank">
    <?php echo __('Update Latest Version','templatic'); ?>
    </a> </div>
  <?php } ?>
  	<div class="tmpl-form">
	  <!-- Logo -->
	  <div id='pblogo'> <a href="http://templatic.com" alt="WordPress Video Theme" title="WordPress Video Theme"><img src="<?php echo esc_url( get_template_directory_uri()."/library/images/templatic-wordpress-themes.png"); ?>" alt="WordPress Video Theme" style="margin-right: 50px;" /></a><?php echo '<h3>'. @$theme_name.' Updates</h3>'; ?> </div>
	  <?php
			if($flag ==1){
				if(isset($warnning_message) && $warnning_message!='')
				{?>
	  <div class='error'>
	    <p><strong><?php echo sprintf(__('%s','templatic'), $warnning_message);?></strong></p>
	  </div>
	  <?php
				  }
				if(!isset($successfull_login) && $successfull_login!=1):?>
	  <p class="info"> <?php echo  sprintf(__('Enter your templatic login credentials to update your %s theme to the latest version.','templatic'),$th_name);?> </p>
	  <form action="<?php echo site_url()."/wp-admin/admin.php?page=templatic_menu";?>" name="" method="post">
	    <table>
	      <tr>
	        <td><label>
	            <?php echo __('User Name', 'templatic')?>
	          </label></td>
	        <td><input type="text" name="templatic_username"  /></td>
	      </tr>
	      <tr>
	        <td><label>
	            <?php echo __('Password', 'templatic')?>
	          </label></td>
	  ...
	  <?php else:								
					 ...
					 echo $th_name.' - theme <a id="TB_closeWindowButton" href="'.$download.'" target="_parent" class="button-secondary">'.__('Update Now','templatic').'</a>';
				 endif;
				 
				 } ?>
	<?php
		if($flag == 0){
			echo '<h3>'.__('You have the latest version of the ','templatic').$th_name.'</h3>';
	        echo '<p>'.sprintf(__('<strong>Your version:</strong> %s','templatic'),$theme_version).'</p>';	
		}
